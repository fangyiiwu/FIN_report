...
def check_text_extractability(reader, check_pages=20):
    ...
    # If 20 pages have < 1000 chars total, it's likely an image scan or garbled
...
def main():
    ...
    output_file = os.path.join(base_dir, "Long_Term_Analysis_3008_v2.md")
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("# Long-Term Financial Resilience Analysis - 3008 (2018-2024) [V2]\n\n")
    ...
    if skipped_count > (total_files / 2):
        with open(output_file, "a", encoding="utf-8") as f:
            f.write("\n\n# CRITICAL: Extraction Limitations\n")
            f.write(f"Most annual reports ({skipped_count}/{total_files}) appear to be image-based or protected.\n")
            f.write("Automatic extraction via this method is not feasible without OCR.\n")
        print("\n[ALERT] Most files are unreadable. Stopping further processing.")
